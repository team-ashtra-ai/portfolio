(() => {
  const signature = {"site": "manufacturing", "interaction": "Capability filter, spec table toggle, facility gallery", "mode": "technical"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_manufacturing');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "manufacturing";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_manufacturing');
