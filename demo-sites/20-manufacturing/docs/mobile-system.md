...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Industrial product specification` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Industrial product specification with full container range |
...
| Mobile hero style | Precision product hero simplified without losing proof or CTA |
| Mobile card stacking | Specification cards stacks or scrolls only when intentional |
| Mobile form behaviour | RFQ spec form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Send RFQ appears after proof and in contact routes |
| Mobile footer structure | capability footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | 3D printers, material samples, production benches |
