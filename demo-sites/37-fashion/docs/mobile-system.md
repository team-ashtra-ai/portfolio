...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Sunlit campaign lookbook` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Sunlit campaign lookbook with full container range |
| Large screen layout | use --container-xl for premium breathing room; avoid simply stretching cards |
| Laptop layout | theatrical density with no overlapping text or controls |
...
| Mobile hero style | Oversized campaign hero simplified without losing proof or CTA |
| Mobile card stacking | Sunny lookbook cards stacks or scrolls only when intentional |
| Mobile form behaviour | Styling enquiry form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Shop Collection appears after proof and in contact routes |
| Mobile footer structure | stockist footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Campaign imagery, fabric closeups, product scale, runway detail |
