(() => {
  const signature = {"site": "fashion", "interaction": "Lookbook slider, size guide, collection filter", "mode": "editorial"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_fashion');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "fashion";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_fashion');
