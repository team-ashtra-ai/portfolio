(() => {
  const signature = {"site": "recruitment", "interaction": "Employer/candidate toggle, role filter, upload CV flow", "mode": "professional"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_recruitment');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "recruitment";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_recruitment');
