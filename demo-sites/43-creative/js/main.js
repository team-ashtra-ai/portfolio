(() => {
  const signature = {"site": "creative", "interaction": "Portfolio filter, project lightbox, file/brief upload", "mode": "editorial"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_creative');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "creative";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_creative');
