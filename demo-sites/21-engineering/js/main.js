(() => {
  const signature = {"site": "engineering", "interaction": "System diagram tabs, compliance checklist, project filter", "mode": "technical"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_engineering');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "engineering";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_engineering');
