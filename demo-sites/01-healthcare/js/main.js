(() => {
  const signature = {"site": "healthcare", "interaction": "Appointment pathway, service filter, reassurance FAQ", "mode": "care"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_healthcare');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "healthcare";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_healthcare');
