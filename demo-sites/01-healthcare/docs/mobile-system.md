...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Scan-first health check journey` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Scan-first health check journey with full container range |
...
| Mobile hero style | Body scan split hero simplified without losing proof or CTA |
| Mobile card stacking | Rounded diagnostic data cards stacks or scrolls only when intentional |
| Mobile form behaviour | Scan booking form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Book Appointment appears after proof and in contact routes |
| Mobile footer structure | care route sitemap stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Body-scan rooms, data visualisations, calm clinical detail |
