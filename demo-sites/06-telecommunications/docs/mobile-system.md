...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Space coverage landing` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Space coverage landing with full container range |
| Large screen layout | use --container-xl for premium breathing room; avoid simply stretching cards |
| Laptop layout | practical density with no overlapping text or controls |
...
| Mobile hero style | Black full-bleed coverage hero simplified without losing proof or CTA |
| Mobile card stacking | Satellite coverage cards stacks or scrolls only when intentional |
| Mobile form behaviour | Availability checker form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Check Coverage appears after proof and in contact routes |
| Mobile footer structure | coverage support footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Satellite fields, night skies, coverage maps, hardware silhouettes |
