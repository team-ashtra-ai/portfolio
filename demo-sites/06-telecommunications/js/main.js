(() => {
  const signature = {"site": "telecommunications", "interaction": "Coverage checker mockup, plan filter, outage alert panel", "mode": "professional"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_telecommunications');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "telecommunications";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_telecommunications');
