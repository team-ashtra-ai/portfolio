...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Minimal EV showroom` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Minimal EV showroom with full container range |
...
| Mobile hero style | Stark vehicle product hero simplified without losing proof or CTA |
| Mobile card stacking | Precise vehicle cards stacks or scrolls only when intentional |
| Mobile form behaviour | Test-drive form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Book Test Drive appears after proof and in contact routes |
| Mobile footer structure | service bay footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | EV silhouettes, interiors, chargers, specification panels |
