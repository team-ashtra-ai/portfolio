(() => {
  const signature = {"site": "automotive", "interaction": "Inventory filter, finance calculator, test-drive form", "mode": "dark"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_automotive');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "automotive";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_automotive');
