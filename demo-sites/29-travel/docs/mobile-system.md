...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Dark destination magazine` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Dark destination magazine with full container range |
...
| Mobile hero style | Immersive trip hero simplified without losing proof or CTA |
| Mobile card stacking | Journey story cards stacks or scrolls only when intentional |
| Mobile form behaviour | Trip planner form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Plan Trip appears after proof and in contact routes |
| Mobile footer structure | destination footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Remote places, maps, itinerary details, travel photography |
