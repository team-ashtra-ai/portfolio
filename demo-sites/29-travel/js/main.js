(() => {
  const signature = {"site": "travel", "interaction": "Trip finder, destination filter, itinerary accordion", "mode": "editorial"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_travel');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "travel";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_travel');
