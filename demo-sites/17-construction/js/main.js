(() => {
  const signature = {"site": "construction", "interaction": "Project estimator form, before/after slider, upload photos", "mode": "professional"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_construction');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "construction";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_construction');
