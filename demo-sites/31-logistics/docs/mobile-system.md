...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Global freight command` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Global freight command with full container range |
...
| Mobile hero style | Shipment dashboard hero simplified without losing proof or CTA |
| Mobile card stacking | Tracking dashboard cards stacks or scrolls only when intentional |
| Mobile form behaviour | Freight quote form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Request Quote appears after proof and in contact routes |
| Mobile footer structure | network footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Ports, shipment dashboards, containers, trade lanes |
