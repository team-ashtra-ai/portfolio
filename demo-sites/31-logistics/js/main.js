(() => {
  const signature = {"site": "logistics", "interaction": "Tracking mockup, quote calculator, network map filter", "mode": "technical"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_logistics');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "logistics";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_logistics');
