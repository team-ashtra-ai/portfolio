...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Legal platform workflow` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Legal platform workflow with full container range |
...
| Mobile hero style | Blue legal software hero simplified without losing proof or CTA |
| Mobile card stacking | Workflow document cards stacks or scrolls only when intentional |
| Mobile form behaviour | Consultation intake form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Book Consultation appears after proof and in contact routes |
| Mobile footer structure | jurisdiction footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Legal dashboards, document checklists, calm office detail |
