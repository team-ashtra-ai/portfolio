(() => {
  const signature = {"site": "ecommerce", "interaction": "Catalogue filter, seller/customer support routing", "mode": "commerce"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_ecommerce');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "ecommerce";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_ecommerce');
