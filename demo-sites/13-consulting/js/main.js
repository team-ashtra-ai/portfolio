(() => {
  const signature = {"site": "consulting", "interaction": "Diagnostic quiz, framework tabs, case filter", "mode": "professional"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_consulting');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "consulting";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_consulting');
