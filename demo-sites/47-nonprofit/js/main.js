(() => {
  const signature = {"site": "nonprofit", "interaction": "Donation selector, impact calculator, volunteer role filter", "mode": "professional"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_nonprofit');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "nonprofit";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_nonprofit');
