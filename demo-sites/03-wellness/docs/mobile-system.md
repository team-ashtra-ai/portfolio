...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Routine commerce shelf` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Routine commerce shelf with full container range |
| Large screen layout | use --container-xl for premium breathing room; avoid simply stretching cards |
| Laptop layout | relaxed density with no overlapping text or controls |
...
| Mobile hero style | Product routine hero simplified without losing proof or CTA |
| Mobile card stacking | Supplement capsule cards stacks or scrolls only when intentional |
| Mobile form behaviour | Routine quiz form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Book Consultation appears after proof and in contact routes |
| Mobile footer structure | self-care journal footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Bright product jars, daily rituals, ingredient callouts |
