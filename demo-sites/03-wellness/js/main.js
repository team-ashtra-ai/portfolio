(() => {
  const signature = {"site": "wellness", "interaction": "Routine builder, service fit quiz, package toggle", "mode": "care"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_wellness');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "wellness";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_wellness');
