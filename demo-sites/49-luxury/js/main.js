(() => {
  const signature = {"site": "luxury", "interaction": "Private enquiry reveal, membership request, concierge stepper", "mode": "luxury"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_luxury');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "luxury";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_luxury');
