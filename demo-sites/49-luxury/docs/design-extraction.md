# Design Extraction

For every reference, extract principles rather than copying visible identity. The fixed ASH-TRA page and section matrix remains unchanged.

## 1. Rolex (direct)

URL: https://www.rolex.com/

| Extraction Field | Principle For This ASH-TRA Site |
| --- | --- |
| Layout archetype | Extract industry-specific luxury, premium & high-end services credibility; translate into Heritage luxury maison rather than copying Rolex. |
| Header structure | Borrow the decision logic of a direct reference, then rebuild as Maison heritage header. |
| Desktop navigation style | Use clear route grouping for fixed ASH-TRA pages with density tuned to spacious. |
| Mobile menu behaviour | Adapt mobile cues into Primary-reference mobile navigation with static-content fallback. |
| Footer structure | Use sitemap completeness and trust routing, rebuilt as private concierge footer. |
| Typography mood | Study the mood, then express it through "Arial Narrow" display with "Trebuchet MS" body and Inter accent labels. |
| Colour palette | Translate reference contrast into the original palette #081f18, #0b5f3a, #d4af37, #f4efe2, #111111. |
| Surface system | Use surface hierarchy ideas only; final surfaces are Luxury with deep green, gold restraint, heritage pacing, and private product focus. |
| Hero type | Use the reference to pressure-test the first screen, then build an original Deep-green private hero. |
| Section rhythm | Extract pacing lessons and apply varied fixed-section composition, not a copied sequence. |
| Card design | Convert card intent into Gold hairline cards with unique spacing, shape, and state rules. |
| Form pattern | Use friction and reassurance cues to refine the original Private access form. |
| CTA flow | Map intent to Primary-reference CTA hierarchy using Request Private Access and primary CTA `Request Private Access`. |
| Asset direction | Translate asset principles into Craft details, watches, private rooms, heritage materials and local ASH-TRA assets. |
| Image treatment | Use crop/contrast lessons without using the reference images, crops, or art direction directly. |
| Icon/illustration style | Extract clarity and hierarchy only; final icon language stays local to MaisonVale. |
| JS interaction ideas | Use as inspiration for Private enquiry reveal, membership request, concierge stepper while preserving readable static content. |
| Motion style | Reduce or reshape motion into Slow heritage reveal with reduced-motion support. |
| Mobile behaviour | Apply mobile task priority to Primary-reference mobile navigation and responsive section rhythm. |
| What must not be copied | Do not copy code, copy, logos, images, brand assets, exact layouts, exact animations, or proprietary identity. |

## 2. Louis Vuitton (direct)

URL: https://www.louisvuitton.com

| Extraction Field | Principle For This ASH-TRA Site |
| --- | --- |
| Layout archetype | Extract industry-specific luxury, premium & high-end services credibility; translate into Heritage luxury maison rather than copying Louis Vuitton. |
| Header structure | Borrow the decision logic of a direct reference, then rebuild as Maison heritage header. |
| Desktop navigation style | Use clear route grouping for fixed ASH-TRA pages with density tuned to spacious. |
| Mobile menu behaviour | Adapt mobile cues into Primary-reference mobile navigation with static-content fallback. |
| Footer structure | Use sitemap completeness and trust routing, rebuilt as private concierge footer. |
| Typography mood | Study the mood, then express it through "Arial Narrow" display with "Trebuchet MS" body and Inter accent labels. |
| Colour palette | Translate reference contrast into the original palette #081f18, #0b5f3a, #d4af37, #f4efe2, #111111. |
| Surface system | Use surface hierarchy ideas only; final surfaces are Luxury with deep green, gold restraint, heritage pacing, and private product focus. |
| Hero type | Use the reference to pressure-test the first screen, then build an original Deep-green private hero. |
| Section rhythm | Extract pacing lessons and apply varied fixed-section composition, not a copied sequence. |
| Card design | Convert card intent into Gold hairline cards with unique spacing, shape, and state rules. |
| Form pattern | Use friction and reassurance cues to refine the original Private access form. |
| CTA flow | Map intent to Primary-reference CTA hierarchy using Request Private Access and primary CTA `Request Private Access`. |
| Asset direction | Translate asset principles into Craft details, watches, private rooms, heritage materials and local ASH-TRA assets. |
| Image treatment | Use crop/contrast lessons without using the reference images, crops, or art direction directly. |
| Icon/illustration style | Extract clarity and hierarchy only; final icon language stays local to MaisonVale. |
| JS interaction ideas | Use as inspiration for Private enquiry reveal, membership request, concierge stepper while preserving readable static content. |
| Motion style | Reduce or reshape motion into Slow heritage reveal with reduced-motion support. |
| Mobile behaviour | Apply mobile task priority to Primary-reference mobile navigation and responsive section rhythm. |
| What must not be copied | Do not copy code, copy, logos, images, brand assets, exact layouts, exact animations, or proprietary identity. |

## 3. Aman (direct)

URL: https://www.aman.com

| Extraction Field | Principle For This ASH-TRA Site |
| --- | --- |
| Layout archetype | Extract industry-specific luxury, premium & high-end services credibility; translate into Heritage luxury maison rather than copying Aman. |
| Header structure | Borrow the decision logic of a direct reference, then rebuild as Maison heritage header. |
| Desktop navigation style | Use clear route grouping for fixed ASH-TRA pages with density tuned to spacious. |
| Mobile menu behaviour | Adapt mobile cues into Primary-reference mobile navigation with static-content fallback. |
| Footer structure | Use sitemap completeness and trust routing, rebuilt as private concierge footer. |
| Typography mood | Study the mood, then express it through "Arial Narrow" display with "Trebuchet MS" body and Inter accent labels. |
| Colour palette | Translate reference contrast into the original palette #081f18, #0b5f3a, #d4af37, #f4efe2, #111111. |
| Surface system | Use surface hierarchy ideas only; final surfaces are Luxury with deep green, gold restraint, heritage pacing, and private product focus. |
| Hero type | Use the reference to pressure-test the first screen, then build an original Deep-green private hero. |
| Section rhythm | Extract pacing lessons and apply varied fixed-section composition, not a copied sequence. |
| Card design | Convert card intent into Gold hairline cards with unique spacing, shape, and state rules. |
| Form pattern | Use friction and reassurance cues to refine the original Private access form. |
| CTA flow | Map intent to Primary-reference CTA hierarchy using Request Private Access and primary CTA `Request Private Access`. |
| Asset direction | Translate asset principles into Craft details, watches, private rooms, heritage materials and local ASH-TRA assets. |
| Image treatment | Use crop/contrast lessons without using the reference images, crops, or art direction directly. |
| Icon/illustration style | Extract clarity and hierarchy only; final icon language stays local to MaisonVale. |
| JS interaction ideas | Use as inspiration for Private enquiry reveal, membership request, concierge stepper while preserving readable static content. |
| Motion style | Reduce or reshape motion into Slow heritage reveal with reduced-motion support. |
| Mobile behaviour | Apply mobile task priority to Primary-reference mobile navigation and responsive section rhythm. |
| What must not be copied | Do not copy code, copy, logos, images, brand assets, exact layouts, exact animations, or proprietary identity. |

## 4. Bang and Olufsen (adjacent)

URL: https://www.bang-olufsen.com

| Extraction Field | Principle For This ASH-TRA Site |
| --- | --- |
| Layout archetype | Extract adjacent-category translation for status and discretion; translate into Heritage luxury maison rather than copying Bang and Olufsen. |
| Header structure | Borrow the decision logic of a adjacent reference, then rebuild as Maison heritage header. |
| Desktop navigation style | Use clear route grouping for fixed ASH-TRA pages with density tuned to spacious. |
| Mobile menu behaviour | Adapt mobile cues into Primary-reference mobile navigation with static-content fallback. |
| Footer structure | Use sitemap completeness and trust routing, rebuilt as private concierge footer. |
| Typography mood | Study the mood, then express it through "Arial Narrow" display with "Trebuchet MS" body and Inter accent labels. |
| Colour palette | Translate reference contrast into the original palette #081f18, #0b5f3a, #d4af37, #f4efe2, #111111. |
| Surface system | Use surface hierarchy ideas only; final surfaces are Luxury with deep green, gold restraint, heritage pacing, and private product focus. |
| Hero type | Use the reference to pressure-test the first screen, then build an original Deep-green private hero. |
| Section rhythm | Extract pacing lessons and apply varied fixed-section composition, not a copied sequence. |
| Card design | Convert card intent into Gold hairline cards with unique spacing, shape, and state rules. |
| Form pattern | Use friction and reassurance cues to refine the original Private access form. |
| CTA flow | Map intent to Primary-reference CTA hierarchy using Request Private Access and primary CTA `Request Private Access`. |
| Asset direction | Translate asset principles into Craft details, watches, private rooms, heritage materials and local ASH-TRA assets. |
| Image treatment | Use crop/contrast lessons without using the reference images, crops, or art direction directly. |
| Icon/illustration style | Extract clarity and hierarchy only; final icon language stays local to MaisonVale. |
| JS interaction ideas | Use as inspiration for Private enquiry reveal, membership request, concierge stepper while preserving readable static content. |
| Motion style | Reduce or reshape motion into Slow heritage reveal with reduced-motion support. |
| Mobile behaviour | Apply mobile task priority to Primary-reference mobile navigation and responsive section rhythm. |
| What must not be copied | Do not copy code, copy, logos, images, brand assets, exact layouts, exact animations, or proprietary identity. |

## 5. NetJets (adjacent)

URL: https://www.netjets.com

| Extraction Field | Principle For This ASH-TRA Site |
| --- | --- |
| Layout archetype | Extract adjacent-category translation for status and discretion; translate into Heritage luxury maison rather than copying NetJets. |
| Header structure | Borrow the decision logic of a adjacent reference, then rebuild as Maison heritage header. |
| Desktop navigation style | Use clear route grouping for fixed ASH-TRA pages with density tuned to spacious. |
| Mobile menu behaviour | Adapt mobile cues into Primary-reference mobile navigation with static-content fallback. |
| Footer structure | Use sitemap completeness and trust routing, rebuilt as private concierge footer. |
| Typography mood | Study the mood, then express it through "Arial Narrow" display with "Trebuchet MS" body and Inter accent labels. |
| Colour palette | Translate reference contrast into the original palette #081f18, #0b5f3a, #d4af37, #f4efe2, #111111. |
| Surface system | Use surface hierarchy ideas only; final surfaces are Luxury with deep green, gold restraint, heritage pacing, and private product focus. |
| Hero type | Use the reference to pressure-test the first screen, then build an original Deep-green private hero. |
| Section rhythm | Extract pacing lessons and apply varied fixed-section composition, not a copied sequence. |
| Card design | Convert card intent into Gold hairline cards with unique spacing, shape, and state rules. |
| Form pattern | Use friction and reassurance cues to refine the original Private access form. |
| CTA flow | Map intent to Primary-reference CTA hierarchy using Request Private Access and primary CTA `Request Private Access`. |
| Asset direction | Translate asset principles into Craft details, watches, private rooms, heritage materials and local ASH-TRA assets. |
| Image treatment | Use crop/contrast lessons without using the reference images, crops, or art direction directly. |
| Icon/illustration style | Extract clarity and hierarchy only; final icon language stays local to MaisonVale. |
| JS interaction ideas | Use as inspiration for Private enquiry reveal, membership request, concierge stepper while preserving readable static content. |
| Motion style | Reduce or reshape motion into Slow heritage reveal with reduced-motion support. |
| Mobile behaviour | Apply mobile task priority to Primary-reference mobile navigation and responsive section rhythm. |
| What must not be copied | Do not copy code, copy, logos, images, brand assets, exact layouts, exact animations, or proprietary identity. |

## 6. Aesop (contrast)

URL: https://www.aesop.com

| Extraction Field | Principle For This ASH-TRA Site |
| --- | --- |
| Layout archetype | Extract contrast-world inspiration to avoid generic luxury design; translate into Heritage luxury maison rather than copying Aesop. |
| Header structure | Borrow the decision logic of a contrast reference, then rebuild as Maison heritage header. |
| Desktop navigation style | Use clear route grouping for fixed ASH-TRA pages with density tuned to spacious. |
| Mobile menu behaviour | Adapt mobile cues into Primary-reference mobile navigation with static-content fallback. |
| Footer structure | Use sitemap completeness and trust routing, rebuilt as private concierge footer. |
| Typography mood | Study the mood, then express it through "Arial Narrow" display with "Trebuchet MS" body and Inter accent labels. |
| Colour palette | Translate reference contrast into the original palette #081f18, #0b5f3a, #d4af37, #f4efe2, #111111. |
| Surface system | Use surface hierarchy ideas only; final surfaces are Luxury with deep green, gold restraint, heritage pacing, and private product focus. |
| Hero type | Use the reference to pressure-test the first screen, then build an original Deep-green private hero. |
| Section rhythm | Extract pacing lessons and apply varied fixed-section composition, not a copied sequence. |
| Card design | Convert card intent into Gold hairline cards with unique spacing, shape, and state rules. |
| Form pattern | Use friction and reassurance cues to refine the original Private access form. |
| CTA flow | Map intent to Primary-reference CTA hierarchy using Request Private Access and primary CTA `Request Private Access`. |
| Asset direction | Translate asset principles into Craft details, watches, private rooms, heritage materials and local ASH-TRA assets. |
| Image treatment | Use crop/contrast lessons without using the reference images, crops, or art direction directly. |
| Icon/illustration style | Extract clarity and hierarchy only; final icon language stays local to MaisonVale. |
| JS interaction ideas | Use as inspiration for Private enquiry reveal, membership request, concierge stepper while preserving readable static content. |
| Motion style | Reduce or reshape motion into Slow heritage reveal with reduced-motion support. |
| Mobile behaviour | Apply mobile task priority to Primary-reference mobile navigation and responsive section rhythm. |
| What must not be copied | Do not copy code, copy, logos, images, brand assets, exact layouts, exact animations, or proprietary identity. |

## 7. Polestar (contrast)

URL: https://www.polestar.com

| Extraction Field | Principle For This ASH-TRA Site |
| --- | --- |
| Layout archetype | Extract contrast-world inspiration to avoid generic luxury design; translate into Heritage luxury maison rather than copying Polestar. |
| Header structure | Borrow the decision logic of a contrast reference, then rebuild as Maison heritage header. |
| Desktop navigation style | Use clear route grouping for fixed ASH-TRA pages with density tuned to spacious. |
| Mobile menu behaviour | Adapt mobile cues into Primary-reference mobile navigation with static-content fallback. |
| Footer structure | Use sitemap completeness and trust routing, rebuilt as private concierge footer. |
| Typography mood | Study the mood, then express it through "Arial Narrow" display with "Trebuchet MS" body and Inter accent labels. |
| Colour palette | Translate reference contrast into the original palette #081f18, #0b5f3a, #d4af37, #f4efe2, #111111. |
| Surface system | Use surface hierarchy ideas only; final surfaces are Luxury with deep green, gold restraint, heritage pacing, and private product focus. |
| Hero type | Use the reference to pressure-test the first screen, then build an original Deep-green private hero. |
| Section rhythm | Extract pacing lessons and apply varied fixed-section composition, not a copied sequence. |
| Card design | Convert card intent into Gold hairline cards with unique spacing, shape, and state rules. |
| Form pattern | Use friction and reassurance cues to refine the original Private access form. |
| CTA flow | Map intent to Primary-reference CTA hierarchy using Request Private Access and primary CTA `Request Private Access`. |
| Asset direction | Translate asset principles into Craft details, watches, private rooms, heritage materials and local ASH-TRA assets. |
| Image treatment | Use crop/contrast lessons without using the reference images, crops, or art direction directly. |
| Icon/illustration style | Extract clarity and hierarchy only; final icon language stays local to MaisonVale. |
| JS interaction ideas | Use as inspiration for Private enquiry reveal, membership request, concierge stepper while preserving readable static content. |
| Motion style | Reduce or reshape motion into Slow heritage reveal with reduced-motion support. |
| Mobile behaviour | Apply mobile task priority to Primary-reference mobile navigation and responsive section rhythm. |
| What must not be copied | Do not copy code, copy, logos, images, brand assets, exact layouts, exact animations, or proprietary identity. |

## 8. Mobbin private access patterns (interaction)

URL: https://mobbin.com

| Extraction Field | Principle For This ASH-TRA Site |
| --- | --- |
| Layout archetype | Extract interaction reference for Private enquiry reveal, membership request, concierge stepper; translate into Heritage luxury maison rather than copying Mobbin private access patterns. |
| Header structure | Borrow the decision logic of a interaction reference, then rebuild as Maison heritage header. |
| Desktop navigation style | Use clear route grouping for fixed ASH-TRA pages with density tuned to spacious. |
| Mobile menu behaviour | Adapt mobile cues into Primary-reference mobile navigation with static-content fallback. |
| Footer structure | Use sitemap completeness and trust routing, rebuilt as private concierge footer. |
| Typography mood | Study the mood, then express it through "Arial Narrow" display with "Trebuchet MS" body and Inter accent labels. |
| Colour palette | Translate reference contrast into the original palette #081f18, #0b5f3a, #d4af37, #f4efe2, #111111. |
| Surface system | Use surface hierarchy ideas only; final surfaces are Luxury with deep green, gold restraint, heritage pacing, and private product focus. |
| Hero type | Use the reference to pressure-test the first screen, then build an original Deep-green private hero. |
| Section rhythm | Extract pacing lessons and apply varied fixed-section composition, not a copied sequence. |
| Card design | Convert card intent into Gold hairline cards with unique spacing, shape, and state rules. |
| Form pattern | Use friction and reassurance cues to refine the original Private access form. |
| CTA flow | Map intent to Primary-reference CTA hierarchy using Request Private Access and primary CTA `Request Private Access`. |
| Asset direction | Translate asset principles into Craft details, watches, private rooms, heritage materials and local ASH-TRA assets. |
| Image treatment | Use crop/contrast lessons without using the reference images, crops, or art direction directly. |
| Icon/illustration style | Extract clarity and hierarchy only; final icon language stays local to MaisonVale. |
| JS interaction ideas | Use as inspiration for Private enquiry reveal, membership request, concierge stepper while preserving readable static content. |
| Motion style | Reduce or reshape motion into Slow heritage reveal with reduced-motion support. |
| Mobile behaviour | Apply mobile task priority to Primary-reference mobile navigation and responsive section rhythm. |
| What must not be copied | Do not copy code, copy, logos, images, brand assets, exact layouts, exact animations, or proprietary identity. |

## Originality Commitment

Inspired by these patterns, this ASH-TRA site will use an original design system with different branding, copy, layout, CSS, assets, and interactions.

## Static Authorship Pass

This site folder is now treated as the editable static source for `MaisonVale`. The duplicate secondary header route strip was removed from the static HTML, and the footer markup/CSS was rewritten directly in the site files using the `immersive` footer pattern for the `Rolex` inspiration direction. Future edits should happen in this site folder rather than regenerating shared templates.

## Portfolio Section Component Pass - 2026-05-11

MaisonVale now treats every main-body section as a static, WordPress-ready component with a component ID, section type, layout variation, pattern family, mobile stacking rule, and editable field map. The pass keeps the Rolex inspiration as a principle reference only while using original local markup, CSS, JS, and assets. Repeated section names are deliberately varied so Hero, Services, Process, Results, FAQ, Contact, and CTA patterns do not collapse into one shared layout.


## Homepage Flow Cleanup Pass - 2026-05-13

- Homepage content now remains hero-first and CTA-final; the notes/disclaimer block is placed before the final CTA so the page closes on `Request Private Access`.
- The former public homepage atlas was removed so the homepage follows the approved section order directly from Hero into the first content section.
- The hero secondary CTA and shortcut chips now point to real homepage sections instead of an inserted route-index block.
- Homepage section text links now route deeper into the site instead of looping back to Home.
- Atlas variant: `civic` for `Maison`, shaped by `Heritage luxury maison`, `Gold hairline cards`, `Private enquiry reveal, membership request, concierge stepper`, and `luxury` mobile behaviour.
- No new external inspiration was added; the existing reference pack remains the source for layout, header, navigation, footer, typography, colour, cards, forms, assets, motion, mobile behaviour, and non-copy rules.


## Portfolio Visual QA Pass - 2026-05-12

- Section copy was tightened so reusable blocks explain `Maison` with clearer role, proof, fit, and next-step language instead of repeated placeholder patterns.
- One-word section headings were rewritten into decision-focused labels so each page scans like a designed journey rather than a content matrix.
- Generic card labels were replaced with section-purpose labels across services, trust, results, editorial, FAQ, and CTA blocks.
- CSS now keeps screenshot and no-JS states visible, disables problematic `content-visibility` reservations for portfolio review, prevents horizontal overflow, and avoids awkward mid-word breaks.
- The pass preserves each site's existing inspiration pack, local assets, component families, section rhythm, and cross-site identity while improving presentation quality across all pages.


## Portfolio Design Refactor Pass - 2026-05-13

- The former public homepage route index is removed so the homepage follows the approved `Luxury, Premium & High-End Services` section flow directly from hero into the first content section.
- The repeated signature widget is visually suppressed to remove duplicated Start/Review/Book panels and reduce hero/section clutter while preserving the static HTML, JS, and routing surface.
- Visible SVG placeholder labels such as asset-type/local-asset annotations were removed so hero and card visuals read as finished industry artwork instead of asset inventory screens.
- Site-local CSS now tightens vertical rhythm, restores real hero grid columns, prevents full-bleed visual layers from sitting over copy, improves card padding, CTA hierarchy, form spacing, FAQ spacing, footer wrapping, and responsive grid collapse across mobile, tablet, desktop, and ultrawide widths.
- The pass keeps the approved references, local assets, page structure, and cross-site difference score while improving visible polish and reducing template-generated repetition.


## Homepage Premium UX Refactor Pass - 2026-05-13

- The homepage now preserves the 10-section flow by removing the extra Important notes section from `index.html`; legal/disclaimer context remains available through utility pages and footer routes.
- The hero is reduced to one eyebrow, one H1, one short lead, and exactly two actions: the site primary CTA plus one secondary Explore route. Repeated hero shortcut navs, proof chips, signature controls, target-stage UI text, and figcaptions were removed from the homepage hero and source hero partial.
- The local homepage hero image set was regenerated as no-visible-text `private atelier` artwork for desktop, tablet, and mobile. The image direction remains tied to `Craft details, watches, private rooms, heritage materials` and does not copy any reference asset.
- Site-local CSS now strengthens premium visual hierarchy, responsive section balance, equal-height card/media groups, readable H1-H6 rhythm, button icon affordances, hover/reveal transitions, and high-contrast footer treatment.
- The pass keeps the approved 8-reference transformation pack and cross-site difference score target of 4-5 while addressing visible polish, proportion, and readability issues.
