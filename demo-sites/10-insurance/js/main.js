(() => {
  const signature = {"site": "insurance", "interaction": "Cover comparison, claims stepper, quote form logic", "mode": "professional"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_insurance');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "insurance";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_insurance');
