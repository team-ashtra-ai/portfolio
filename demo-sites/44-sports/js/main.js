(() => {
  const signature = {"site": "sports", "interaction": "Timetable filter, membership toggle, coach selector", "mode": "professional"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_sports');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "sports";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_sports');
