...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Athletic product-program system` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Athletic product-program system with full container range |
...
| Mobile hero style | Big sport campaign hero simplified without losing proof or CTA |
| Mobile card stacking | Performance program cards stacks or scrolls only when intentional |
| Mobile form behaviour | Trial class form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Join Class appears after proof and in contact routes |
| Mobile footer structure | community footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Athletes, training spaces, shoes, performance metrics |
