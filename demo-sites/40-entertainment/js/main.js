(() => {
  const signature = {"site": "entertainment", "interaction": "Event calendar, ticket selector, artist filter", "mode": "dark"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_entertainment');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "entertainment";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_entertainment');
