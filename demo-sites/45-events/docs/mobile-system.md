...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Clean event calendar system` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Clean event calendar system with full container range |
...
| Mobile hero style | Event card hero simplified without losing proof or CTA |
| Mobile card stacking | RSVP event cards stacks or scrolls only when intentional |
| Mobile form behaviour | Date enquiry form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Enquire Date appears after proof and in contact routes |
| Mobile footer structure | venue and supplier footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Event cards, calendars, guest lists, venue details |
