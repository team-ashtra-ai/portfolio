(() => {
  const signature = {"site": "events", "interaction": "Event type selector, venue filter, package calculator", "mode": "hospitality"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_events');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "events";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_events');
