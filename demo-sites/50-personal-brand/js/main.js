(() => {
  const signature = {"site": "personal-brand", "interaction": "Media filter, speaking topic selector, press kit download", "mode": "editorial"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_personal-brand');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "personal-brand";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_personal-brand');
