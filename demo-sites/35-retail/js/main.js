(() => {
  const signature = {"site": "retail", "interaction": "Product/category filter, loyalty signup, store finder", "mode": "commerce"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_retail');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "retail";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_retail');
