...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Minimal product store` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Minimal product store with full container range |
...
| Mobile hero style | Soft product shelf hero simplified without losing proof or CTA |
| Mobile card stacking | Rounded product tiles stacks or scrolls only when intentional |
| Mobile form behaviour | Store finder form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Visit Shop appears after proof and in contact routes |
| Mobile footer structure | store finder footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Product tiles, store spaces, service cards, device-like panels |
