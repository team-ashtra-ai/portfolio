(() => {
  const signature = {"site": "publishing", "interaction": "Library search, author filter, newsletter modal", "mode": "editorial"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_publishing');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "publishing";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_publishing');
