...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Print magazine archive` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Print magazine archive with full container range |
...
| Mobile hero style | Editorial masthead hero simplified without losing proof or CTA |
| Mobile card stacking | Column article cards stacks or scrolls only when intentional |
| Mobile form behaviour | Newsletter form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Subscribe appears after proof and in contact routes |
| Mobile footer structure | archive footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Magazine spreads, desks, books, archive covers |
