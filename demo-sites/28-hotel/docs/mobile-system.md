...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Quiet retreat booking flow` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Quiet retreat booking flow with full container range |
...
| Mobile hero style | Serene full-bleed stay hero simplified without losing proof or CTA |
| Mobile card stacking | Retreat room cards stacks or scrolls only when intentional |
| Mobile form behaviour | Availability form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Check Availability appears after proof and in contact routes |
| Mobile footer structure | stay support footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Villas, landscapes, spa spaces, calm interiors |
