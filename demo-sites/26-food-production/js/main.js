(() => {
  const signature = {"site": "food-production", "interaction": "Product/allergen filter, stockist finder, recipe tabs", "mode": "commerce"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_food-production');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "food-production";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_food-production');
