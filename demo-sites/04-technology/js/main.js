(() => {
  const signature = {"site": "technology", "interaction": "Support plan comparison, system status cards, ticket route selector", "mode": "technical"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_technology');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "technology";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_technology');
