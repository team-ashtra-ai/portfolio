...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Edge network product system` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Edge network product system with full container range |
...
| Mobile hero style | Network diagram hero simplified without losing proof or CTA |
| Mobile card stacking | Network service tiles stacks or scrolls only when intentional |
| Mobile form behaviour | IT audit route form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Request IT Audit appears after proof and in contact routes |
| Mobile footer structure | support desk footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Edge maps, server nodes, operational dashboards |
