...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Apothecary routine journal` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Apothecary routine journal with full container range |
...
| Mobile hero style | Muted product education hero simplified without losing proof or CTA |
| Mobile card stacking | Ingredient education cards stacks or scrolls only when intentional |
| Mobile form behaviour | Routine advice form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Build Routine appears after proof and in contact routes |
| Mobile footer structure | ingredient footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Bottles, textures, counters, botanical ingredient details |
