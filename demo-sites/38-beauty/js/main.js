(() => {
  const signature = {"site": "beauty", "interaction": "Routine finder, ingredient glossary, product filter", "mode": "commerce"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_beauty');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "beauty";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_beauty');
