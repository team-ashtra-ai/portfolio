...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Map-first route planner` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Map-first route planner with full container range |
...
| Mobile hero style | Green transit planner hero simplified without losing proof or CTA |
| Mobile card stacking | Transit route cards stacks or scrolls only when intentional |
| Mobile form behaviour | Fare estimate form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Book Ride appears after proof and in contact routes |
| Mobile footer structure | fleet support footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Transit maps, route lines, vehicles, city movement |
