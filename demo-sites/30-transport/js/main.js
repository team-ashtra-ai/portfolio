(() => {
  const signature = {"site": "transport", "interaction": "Route selector, fare estimate, fleet filter", "mode": "professional"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_transport');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "transport";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_transport');
