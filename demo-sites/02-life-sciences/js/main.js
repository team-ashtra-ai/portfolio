(() => {
  const signature = {"site": "life-sciences", "interaction": "Pipeline tabs, publication filters, trial eligibility toggle", "mode": "technical"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_life-sciences');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "life-sciences";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_life-sciences');
