(() => {
  const signature = {"site": "finance", "interaction": "Advisory route selector, calculator-style estimate, risk toggles", "mode": "professional"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_finance');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "finance";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_finance');
