...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Green calculator flow` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Green calculator flow with full container range |
| Large screen layout | use --container-xl for premium breathing room; avoid simply stretching cards |
| Laptop layout | measured density with no overlapping text or controls |
...
| Mobile hero style | Money movement calculator hero simplified without losing proof or CTA |
| Mobile card stacking | Fee transparency cards stacks or scrolls only when intentional |
| Mobile form behaviour | Advisor calculator form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Book Advisor Call appears after proof and in contact routes |
| Mobile footer structure | trust/legal footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Payment flows, rate cards, account panels |
