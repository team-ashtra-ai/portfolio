(() => {
  const signature = {"site": "government", "interaction": "Service search, form finder, document filter, alert controls", "mode": "civic"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_government');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "government";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_government');
