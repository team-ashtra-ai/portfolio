...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Public service task flow` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Public service task flow with full container range |
...
| Mobile hero style | Plain service hero simplified without losing proof or CTA |
| Mobile card stacking | Square service boxes stacks or scrolls only when intentional |
| Mobile form behaviour | Public request form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Start Request appears after proof and in contact routes |
| Mobile footer structure | service directory footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Forms, service lists, notices, public task flows |
