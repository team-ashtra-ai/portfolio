(() => {
  const signature = {"site": "energy", "interaction": "Savings estimator, battery/solar toggle, project output counters", "mode": "technical"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_energy');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "energy";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_energy');
