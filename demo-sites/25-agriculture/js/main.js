(() => {
  const signature = {"site": "agriculture", "interaction": "Seasonal calendar, product filter, advice route selector", "mode": "professional"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_agriculture');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "agriculture";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_agriculture');
