(() => {
  const signature = {"site": "data-analytics", "interaction": "Dashboard tabs, KPI filters, tool stack selector", "mode": "technical"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_data-analytics');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "data-analytics";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_data-analytics');
