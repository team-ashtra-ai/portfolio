...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Context layer data workspace` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Context layer data workspace with full container range |
...
| Mobile hero style | Metadata dashboard hero simplified without losing proof or CTA |
| Mobile card stacking | Collaborative data cards stacks or scrolls only when intentional |
| Mobile form behaviour | Dashboard planning form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Plan Dashboard appears after proof and in contact routes |
| Mobile footer structure | data library footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Data catalogues, lineage graphs, metadata workspaces |
