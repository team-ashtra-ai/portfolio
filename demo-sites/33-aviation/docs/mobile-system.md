...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Supersonic operations deck` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Supersonic operations deck with full container range |
...
| Mobile hero style | Aerospace velocity hero simplified without losing proof or CTA |
| Mobile card stacking | Fleet specification cards stacks or scrolls only when intentional |
| Mobile form behaviour | Charter route form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Request Charter appears after proof and in contact routes |
| Mobile footer structure | compliance footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Aircraft, runways, hangars, flight paths |
