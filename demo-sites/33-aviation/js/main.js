(() => {
  const signature = {"site": "aviation", "interaction": "Fleet specs tabs, safety checklist, charter route form", "mode": "dark"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_aviation');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "aviation";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_aviation');
