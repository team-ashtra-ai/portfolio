(() => {
  const signature = {"site": "architecture", "interaction": "Project gallery, planning stage stepper, image reveal", "mode": "editorial"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_architecture');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "architecture";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_architecture');
