...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Spatial project index` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Spatial project index with full container range |
...
| Mobile hero style | Minimal full-bleed project hero simplified without losing proof or CTA |
| Mobile card stacking | Architectural index cards stacks or scrolls only when intentional |
| Mobile form behaviour | Project brief form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Discuss Project appears after proof and in contact routes |
| Mobile footer structure | project index footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Architecture photography, plans, models, site context |
