...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Modern clinic care flow` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Modern clinic care flow with full container range |
...
| Mobile hero style | Warm appointment hero simplified without losing proof or CTA |
| Mobile card stacking | Rounded care cards stacks or scrolls only when intentional |
| Mobile form behaviour | Appointment form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Book Visit appears after proof and in contact routes |
| Mobile footer structure | care guide footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Clinic rooms, pets, care teams, appointment panels |
