(() => {
  const signature = {"site": "veterinary", "interaction": "Emergency symptom helper, appointment form, care guide filter", "mode": "care"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_veterinary');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "veterinary";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_veterinary');
