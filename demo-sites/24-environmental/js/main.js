(() => {
  const signature = {"site": "environmental", "interaction": "Impact metric filters, ESG report archive, audit route selector", "mode": "technical"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_environmental');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "environmental";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_environmental');
