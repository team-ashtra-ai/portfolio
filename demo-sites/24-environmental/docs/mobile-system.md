...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Climate data operating system` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Climate data operating system with full container range |
...
| Mobile hero style | Emissions dashboard hero simplified without losing proof or CTA |
| Mobile card stacking | Carbon evidence cards stacks or scrolls only when intentional |
| Mobile form behaviour | Audit advisory form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Book Audit appears after proof and in contact routes |
| Mobile footer structure | report archive footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Carbon dashboards, climate reports, data rooms |
