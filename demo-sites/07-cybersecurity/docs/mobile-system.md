...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Cloud security command centre` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Cloud security command centre with full container range |
...
| Mobile hero style | Purple cloud-risk hero simplified without losing proof or CTA |
| Mobile card stacking | Rounded cloud-risk cards stacks or scrolls only when intentional |
| Mobile form behaviour | Audit request form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Request Audit appears after proof and in contact routes |
| Mobile footer structure | security evidence footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Cloud graphs, risk paths, security dashboards |
