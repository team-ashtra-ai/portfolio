(() => {
  const signature = {"site": "cybersecurity", "interaction": "Risk matrix, compliance checklist, incident response stepper", "mode": "dark"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_cybersecurity');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "cybersecurity";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_cybersecurity');
