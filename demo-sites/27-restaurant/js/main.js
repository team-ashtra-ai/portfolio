(() => {
  const signature = {"site": "restaurant", "interaction": "Menu filter, allergen toggle, reservation widget, gallery", "mode": "hospitality"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_restaurant');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "restaurant";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_restaurant');
