...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Cloud accounting dashboard` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Cloud accounting dashboard with full container range |
| Large screen layout | use --container-xl for premium breathing room; avoid simply stretching cards |
| Laptop layout | compact density with no overlapping text or controls |
...
| Mobile hero style | Rounded ledger dashboard hero simplified without losing proof or CTA |
| Mobile card stacking | Ledger dashboard cards stacks or scrolls only when intentional |
| Mobile form behaviour | Filing quote form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Request Quote appears after proof and in contact routes |
| Mobile footer structure | filing support footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Bookkeeping dashboards, invoices, payroll cards |
