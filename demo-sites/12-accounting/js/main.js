(() => {
  const signature = {"site": "accounting", "interaction": "Deadline calendar, document checklist, package comparison", "mode": "professional"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_accounting');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "accounting";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_accounting');
