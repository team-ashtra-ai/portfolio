(() => {
  const signature = {"site": "utilities", "interaction": "Service request finder, alert banner, report filter", "mode": "civic"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_utilities');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "utilities";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_utilities');
