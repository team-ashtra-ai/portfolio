...
The mobile experience must not be the same default stack everywhere. This site uses `Primary-reference mobile navigation` and `Playful utility account flow` to control mobile rhythm.

| Breakpoint/Area | Decision |
| --- | --- |
| Desktop layout | Playful utility account flow with full container range |
...
| Mobile hero style | Purple service hero simplified without losing proof or CTA |
| Mobile card stacking | Rounded tariff cards stacks or scrolls only when intentional |
| Mobile form behaviour | Service request form keeps labels visible and submits to Formspree |
| Mobile CTA placement | Request Service appears after proof and in contact routes |
| Mobile footer structure | public service footer stacks as sitemap, contact, legal, and trust |
| Mobile image cropping | Energy accounts, meters, tariff calculators, service notices |
