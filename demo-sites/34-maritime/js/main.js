(() => {
  const signature = {"site": "maritime", "interaction": "Vessel filter, port schedule, cargo quote form", "mode": "dark"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_maritime');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "maritime";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_maritime');
