(() => {
  const signature = {"site": "real-estate", "interaction": "Property filter, gallery modal, map-style area cards", "mode": "professional"};
  ...
  if (acceptCookie && cookieBanner) acceptCookie.addEventListener('click', () => {
    ...
    track('consent_real-estate');
  });
  idle(() => {
    document.documentElement.dataset.staticSite = "real-estate";
    document.documentElement.dataset.footerPattern = document.querySelector('.hand-footer')?.className.match(/hand-footer-(\w+)/)?.[1] || 'site';
    if (body.dataset.assetSystem === 'true') track('asset_system_view_real-estate');
